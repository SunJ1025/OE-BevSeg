...
# -------------------------------------------- vov-encoder ---------------------------- #
from nets.vovnet import VoVNet


class Encoder_vov(nn.Module):
    def __init__(self, C):
        super().__init__()
        self.C = C
        self.backbone = VoVNet(spec_name='V-99-eSE',
                               input_ch=3,
                               ...
                               pretrained=None,
                               init_cfg=None)
        state_dict = torch.load('nets/vov99_new_weights.pth')
        # self.backbone.load_state_dict(state_dict)

        pretrained_dict = state_dict
        modified_model_dict = self.backbone.state_dict()

        pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in modified_model_dict and 'mamba' not in k}
        modified_model_dict.update(pretrained_dict)
        self.backbone.load_state_dict(modified_model_dict)

        self.depth_layer = nn.Conv2d(512, self.C, kernel_size=1, padding=0)
        self.upsampling_layer = UpsamplingConcat(1280, 512)

    def forward(self, x):
        out = self.backbone(x)  # [2, 512, 56, 100] [2, 768, 28, 50]
        x = self.upsampling_layer(out['stage4'], out['stage3'])
...
class Segnet(nn.Module):
    def __init__(self, Z, Y, X, vox_util=None,
        ...
        assert (encoder_type in ["res101", "res50", "effb0", "effb4", "vov99"])
        ...
        # --------------  添加 mamba layer ----------------------#
        self.mamba_layer = MambaLayer(dim=1024)
        ...
        # ------------ 添加 vov --------------- #
        elif encoder_type == "vov99":
            self.encoder = Encoder_vov(feat2d_dim)
        # ------------ 添加 vov --------------- #
    ...
    def forward(self, rgb_camXs, pix_T_cams, cam0_T_camXs, rad_occ_mem0=None):
        ...
        if self.use_radar:
            ...
            if self.do_rgbcompress:
                ...
                ffff = feat_bev_
                # --------------------  添加 mamba 层 ------------------------- #
                feat_bev_ = self.mamba_layer(feat_bev_)
                # feat_bev_ = self.dropout(feat_bev_)
                # --------------------  添加 mamba 层 ------------------------- #
                feat_bev = self.bev_compressor(feat_bev_)

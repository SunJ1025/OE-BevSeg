...
def extract_rectangular_annular_regions(input_tensor):
    ...
    assert input_tensor.shape == (1, 1, 200, 200), "Input tensor shape must be [1, 1, 200, 200]"

    h, w = input_tensor.shape[2], input_tensor.shape[3]
    center = (w // 2, h // 2)

    # 创建矩形掩码
    mask_80 = create_rectangular_mask(h, w, center[0] - 40, center[0] + 40)
    mask_140 = create_rectangular_mask(h, w, center[0] - 70, center[0] + 70)
    mask_200 = create_rectangular_mask(h, w, center[0] - 100, center[0] + 100)

    # 转换掩码为张量
    mask_80 = torch.from_numpy(mask_80).unsqueeze(0).unsqueeze(0).float().cuda()
    mask_140 = torch.from_numpy(mask_140).unsqueeze(0).unsqueeze(0).float().cuda()
    mask_200 = torch.from_numpy(mask_200).unsqueeze(0).unsqueeze(0).float().cuda()
...
def run_model(model, loss_fn, d, device='cuda:0', sw=None):  #
    ...
    # # 使用ptflops计算GFLOPS
    # flops, params = profile(model.to('cpu'), inputs=(rgb_camXs.to('cpu')))
    # # 将结果转换为更易于阅读的格式
    # flops, params = clever_format([flops, params], '%.3f')
    # print(f"运算量：{flops}, 参数量：{params}")

    # from fvcore.nn import FlopCountAnalysis, flop_count_table
    # flops = FlopCountAnalysis(model, (rgb_camXs, pix_T_cams, cam0_T_camXs, in_occ_mem0))
    #
    # # 打印FLOPS表格
    ...
    # ------------------------------ add range intervals -------------------------------- #
    # 提取区域
    # region_80_e, region_80_140_e, region_140_200_e = extract_rectangular_annular_regions(seg_bev_e)
    # region_80_g, region_80_140_g, region_140_200_g = extract_rectangular_annular_regions(seg_bev_g)
    # # 设置区域
    # seg_bev_e = region_80_140_e
    # seg_bev_g = region_80_140_g
    # ------------------------------ add range intervals -------------------------------- #

    seg_bev_e_round = torch.sigmoid(seg_bev_e).round()  # [1, 1, 200, 200]
    # intersection = (seg_bev_e_round * seg_bev_g * valid_bev_g).sum()
    # union = ((seg_bev_e_round + seg_bev_g) * valid_bev_g).clamp(0, 1).sum()

    intersection = (seg_bev_e_round * seg_bev_g).sum()
    union = (seg_bev_e_round + seg_bev_g).clamp(0, 1).sum()

    # --------------------- add F1 ------------------------ #
    seg_bev_e_round = seg_bev_e_round.cpu().numpy()
    seg_bev_g = seg_bev_g.cpu().numpy()
    TP, TN, FP, FN = calculate_metrics(seg_bev_e_round, seg_bev_g)
    metrics['TP'] = TP
    metrics['TN'] = TN
    metrics['FP'] = FP
    metrics['FN'] = FN
...
def main(
    ...
    # model = Segnet_m(Z, Y, X, vox_util, use_radar=use_radar, use_lidar=use_lidar, use_metaradar=use_metaradar,
    #                do_rgbcompress=do_rgbcompress, encoder_type=encoder_type)

    # model = Segnet_m_cen(Z, Y, X, vox_util, use_radar=use_radar, use_lidar=use_lidar, use_metaradar=use_metaradar,
    #                  do_rgbcompress=do_rgbcompress, encoder_type=encoder_type)

    model = Segnet_m_cen_deform(Z, Y, X, vox_util, use_radar=use_radar, use_lidar=use_lidar, use_metaradar=use_metaradar,
                     do_rgbcompress=do_rgbcompress, encoder_type=encoder_type)
    # ----------------------------------- set different models ------------------------------------ #
    ...
    # load checkpoint
    _ = saverloader.load(init_dir, model.module, ignore_load=ignore_load, step=step)
    global_step = 0
    ...
    print('accuracy %.5f; precision %.5f; recall %.5f; f1_score %.5f; dice %.5f;' % (accuracy, precision, recall, f1_score, dice))

...
def run_model(loader, index, model, d, img_dir, device='cuda:0', sw=None):
    ...
    for t in range(T):
        ...
        _, feat_bev_e, seg_bev_e, center_bev_e, offset_bev_e = model(
            ...
            cam0_T_camXs=cam0_T_camXs,
            # vox_util=vox_util,
            rad_occ_mem0=in_occ_mem0)  # feat_bev_e [1, 128, 200, 200] seg [1, 1, 200, 200]
        ...
        # cv2.imwrite('seg——e.jpg',seg_bev_e)
        ...
        target_layers = model.module.decoder.segmentation_head
        ...
        for cam_id in range(n_cam):
            ...
            cv2.imwrite(f'seg——gradcam_cam_{t}.jpg', cam_image)
...
def main(
        ...
        keep_latest=1,
        init_dir='/home/oem/桌面/code_bev_seg/simple_bev-main/checkpoints/8x5_5e-4_rgb12_simple',
        ...
        nsweeps=3,
        # model
        encoder_type='res101',  # vov99 res101
        use_radar=False,  # True
